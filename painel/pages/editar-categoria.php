<?php 
	if(isset($_GET['id'])){
		$id = (int)$_GET['id'];
		$categoria = Painel::select(`projeto_01`.'tb_site.categorias','id = ?',array($id));
	}else{
	...
	<form method="post" enctype="multipart/form-data">
		...
		<?php
			if(isset($_POST['acao'])){
				...
				$verificar = MySql::conectar()->prepare("SELECT * FROM `projeto_01`.`tb_site.categorias` WHERE nome = ? AND id != ?");
